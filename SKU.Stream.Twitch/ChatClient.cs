...
namespace SKU.Stream.Twitch
{
    ...
    {
        public const string LOGGER_CATEGORY = "SKU.Stream.TWichChat";
        private TcpClient _TcpClient;
        private StreamReader inputStream;
        private StreamWriter outputStream;
        private int _Retries;
        private Task _RecieveMessagesTask;
        ...
        private int _RemainingThrottleCommands;
        // private static readonly ReaderWriterLockSlim;

            public ChatClient(IOptions<ConfigurationSettings> settings, ILoggerFactory loggerFactory) : 
            this(settings.Value, loggerFactory.CreateLogger(LOGGER_CATEGORY))
        {
        ...
        {
            var fullMessage = $":{Settings.ChatBotName}!{Settings.ChatBotName}@{Settings.ChatBotName}.tmi.twitch.tv PRIMSG {Settings.ChannelName} :{message}";
            SendMessage(fullMessage);
        }

        public void WhisperMessage(string message, string userName)
        {
            var fullMessage = $":{Settings.ChatBotName}!{Settings.ChatBotName}@{Settings.ChatBotName}.tmi.twitch.tv PRIMSG #jtv :/w {userName} :{message}";
            SendMessage(fullMessage);
        }
        ...
        private void ReceiveMessagesOnThread()
        {
            ...
            while (true)
            {
                ...
                {
                    Logger.LogError($"Haven't recieved a message in {errorPeriod.TotalSeconds} seconds");
        ...
        private void ProcessMessage(string msg)
        {
            ...
            // Review Messages send to the channel
